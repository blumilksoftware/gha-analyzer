...
function parseLineToLog (line) {
    ...
    if (!dictionaries.authors[author]) {
        ...
        dictionaries.authors = JSON.parse(JSON.stringify(dictionaries.authors))
    }
...
function filterLogsBy (tag) {
    if (tables.logs.sort === tag) {
        tables.logs.order = tables.logs.order === 'desc' ? 'asc' : 'desc'
        return
    }

    tables.logs.sort = tag
    tables.logs.order = 'desc'
}
